...
<script>
    ...
    export default {
        ...
        data(){
            return{
                ...
                columnData:[
                    ...
                    // { prop: 'totalAmountReceived', label: '应收合计'}
                ],
                afterData: [
                ...
                lastData: [
                    { prop: 'totalAmountReceived', label: '应收合计', format: 'fmoney'}
                ],
                startPickerOptions: {
                    disabledDate: (time) => {
                        if (this.endMonth) {
        ...
        methods:{
            getSummaries(param) {
                this.$api.reportapi.listSumUsingGET_2(param).then(res=>{
                    if (res.data.status === 200) {
                        let dataTotal = res.data.data;
                        let data = {
                            contractCode: '合计',
                            salesAmount: dataTotal.salesAmount,
                            totalAmountReceived: dataTotal.totalAmount
                        };
            ...
            pageHandler(pageNum, pageSize){
                ...
                if(this.startMonth && this.endMonth){
                    ...
                    this.$api.reportapi.listUsingGET_3(params).then(res=>{
                        if(res.data.status === 200){
            ...
            exportHandler(){
                ...
                if(this.datalist.length>0){
                    this.$api.reportapi.exportBusinessSettlementReceivedUsingGET(params).then(res=>{
                        if(res.data.status == 200){
