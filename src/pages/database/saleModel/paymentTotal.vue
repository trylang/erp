<template>
  <con-head title="付款方式汇总" v-loading.fullscreen="loading">
  ...
  data() {
    return {
      ...
      startpickerOptions: {
        disabledDate: time => {
          if (this.query.endDateData != "" || this.query.endDateData != null) {
            ...
            let oneYearNum =
              new Date(this.query.endDateData).getTime() - oneYear;
      ...
      endpickerOptions: {
        disabledDate: time => {
          if (
            this.query.startDateData != "" ||
            this.query.startDateData != null
          ) {
            let oneYear = 365 * 24 * 3600 * 1000;
            let oneYearNum =
              new Date(this.query.startDateData).getTime() + oneYear;
  ...
  methods: {
      ...
      if (params.startDate && params.endDate) {
        ...
        this.$api.reportapi.shopPayListUsingGET(params).then(returnObj => {
          if (returnObj.data.status === 200) {
            this.$api.reportapi
              .shopPaymentListUsingGET(params)
              .then(res => {
                const data = res.data;
                if (data.status === 200) {
                  this.afterTitles = [];
                  this.header = this.header.slice(0, 7);
    ...
    exportHandler() {
      ...
      if (this.content.list.length > 0 && params.startDate && params.endDate) {
        this.$api.reportapi
          .exportShopPaymentListUsingGET(params)
          .then(res => {
    ...
    async init() {
      this.header = this.header.concat(this.afterTitles);
      let [shop] = await Promise.all([saleQueryShop()]);
