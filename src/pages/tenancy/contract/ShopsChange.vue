                ...
                <data-table :tableData="dataList" :colConfigs="columnData">
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            <router-link :to="'/inner/mainedit/'+prototypeId+'/'+scope.row.id" class="btn_text" v-if="scope.row.status == 30 || scope.row.status == 40 || scope.row.status == 52">主体变更</router-link>
                            <router-link :to="'/inner/areaedit/'+prototypeId+'/'+scope.row.id" class="btn_text" v-if="scope.row.status == 30 || scope.row.status == 40 || scope.row.status == 51">面积变更</router-link>
                        </template>
                    </el-table-column>
                </data-table>
            </div>
    ...
    export default {
        ...
        methods:{
            async getDataList(pageNum,pageSize){
                ...
                this.loading = true;
                await this.$api.rentapi.getContentChangeList({
