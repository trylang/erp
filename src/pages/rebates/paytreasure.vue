  ...
  methods: {
    exportList() {
      ...
      if(this.content.list.length>0){
          this.$api.refundapi.toExcelUsingGET(params).then(res=>{
    ...
    async getList(page={}) {
      ...
      this.loading = true;
      await this.$api.refundapi.getListForPageUsingGET(this.query).then(returnObj => {
        if (returnObj.data.status === 200) {
          ...
          if (returnObj.data.data.isLastPage) {
            this.$api.refundapi.billTotalUsingGET(this.query).then(returnObj => {
              if (returnObj.data.status === 200) {             
