    ...
    <div v-loading.fullscreen="loading">
        <con-head tab="tab">
            <div slot="appendtab" class="tabmenu">
                <router-link to="/rebates/zhxbill" v-if="zhxbill">资和信账单</router-link>
                <router-link to="/rebates/zhxbillDetailed" v-if="zhxbillDetailed">资和信账单明细</router-link>
            </div>
                  ...
                  <div class="texttitle">
                    ...
                    <div class="line-nav">
                      <a href="javascript:void(0)" 
                        v-for="card in selects.cards" 
                        :key="card.id" 
                        :class="{active:card.isStatus}" 
                        @click="cardHandler(card)">{{card.label}}</a>
                    </div>
                  </div>
                </el-col>
                <el-col :span="11" :offset="1">
...
export default {
  ...
  computed:{
    zhxbill(){
        return this.$root.menus.indexOf('/rebates/zhxbill') >= 0;
    },
    zhxbillDetailed(){
        return this.$root.menus.indexOf('/rebates/zhxbillDetailed') >= 0;
    }
  },
  methods: {
    exportetail() {
      ...
      let params = {
              ...
              terminalNumFrom: this.query2.terminalNumFrom,
              terminalNumTo: this.query2.terminalNumTo,
              cardType: this.query2.cardType,
              tradeType: this.query2.tradeType
          }
          if(this.content2.list.length>0){
              this.$api.refundapi.detailToExcelUsingGET_4(params).then(res=>{
