  ...
  <div v-loading.fullscreen="loading">
    <con-head tab="tab">
        <div slot="appendtab" class="tabmenu">
          <router-link to="/rebates/wechat" v-if="wechat">微信账单</router-link>
          <router-link to="/rebates/wechatDetailed" v-if="wechatDetailed">微信账单明细</router-link>
        </div>
...
export default {
  ...
  computed:{
    wechat(){
        return this.$root.menus.indexOf('/rebates/wechat') >= 0;
    },
    wechatDetailed(){
        return this.$root.menus.indexOf('/rebates/wechatDetailed') >= 0;
    }
  },
  methods: {
    exportList() {
      ...
      let params = {
        ...
        if(this.content.list.length>0){
            this.$api.refundapi.toExcelUsingGET_3(params).then(res=>{
    ...
    async getList(page={}) {
      ...
      this.loading = true;
      await this.$api.refundapi.getListForPageUsingGET_8(this.query).then(returnObj => {
