<template>
  <div class="savebox">
    <div class="savecont">
          ...
          <el-row slot="preappend">
            <el-col :span="9">
              <div class="searchselect">
                  ...
                  <el-select v-model="query.merchantId" placeholder="商户名称" filterable clearable class="dialogselect" @change="checkContractHandler(query.merchantId)" :disabled="!!this.$route.query.merchantId">
                    <el-option
            ...
            <el-col :span="9" :offset="6">
              <div class="searchselect">
                <span class="inputname inputnameauto">意向合同</span>
                <el-select v-model="query.contractCode" placeholder="请选择合同" filterable clearable class="dialogselect" @change="checkMoneyHandler" :disabled="!!this.$route.query.contractCode">
                  <el-option
        ...
        <erp-dialog v-loading.fullscreen="dialog.loading" :title="dialog.param.id? '修改诚意金': '诚意金收取'" :dialog="dialog"></erp-dialog>
    </div>
...
export default {
  ...
  mounted() {
    ...
    this.$api.systemapi.typeListUsingGET({ nameOrCode: "0005" }).then(res => {
      //类型code
      let paymentCode = res.data.data[0].code;
      this.$api.systemapi.itemListUsingGET({ code: paymentCode }).then(res => {
        //根据code查询付款方式
  ...
  methods: {
    ...
    async addEntering() {
      ...
      const param = {
        merchantId: this.query.merchantId,
        contractCode: this.query.contractCode,
        shopId: this.query.shopId || null,
        stage: this.query.stage,
        receiptNumber: this.$route.query.receiptNumber
          ? this.querys.receiptNumber
          : null, //编辑传收款单号，新增不用传
        id: this.query.bondId, //保证金id
        list: this.content.list
      };
      const apiFunc = (api, param) => {
        let aTotal = param.list.reduce((a, b) => {
          return a + Number(b.receivedAmount);
        }, 0);
        if (aTotal <= this.content.uncollectedAmount) {
          //收取金额必须小于未收金额
