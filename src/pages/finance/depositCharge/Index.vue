<template>
    <con-head tab="tab">
        <div slot="appendtab" class="tabmenu">
            <router-link to="/finance/depositcharge" v-if="depositCharge">押金收取管理</router-link>
            <router-link to="/finance/depositchargeaudit" v-if="depositChargeAudit">押金收取审核</router-link>
        </div>
        <el-button type="primary" icon="el-icon-plus" slot="append"
                   @click="dialog.dialogVisible = true, dialog.param={merchantId: '',contractItem: '',paymentCode: '', receivedAmount: '',receivedDate: ''}">
            收取
        </el-button>
        <el-row slot="preappend">
            <el-col :span="9">
                <div class="searchbox">
                    <input type="text" placeholder="请输入收款单号" v-model="searchName" @keyup.enter="getAdvanceList()"><i
            ...
            <el-col :span="9" :offset="6">
                <div class="searchselect">
                    ...
                    <el-select v-model="query.merchantId" placeholder="请选择商户名称" filterable clearable
                               @change="checkShopNameList(query.merchantId)" class="dialogselect">
        ...
        <el-row slot="preappend">
            <el-col :span="9">
                <div class="texttitle">
                    ...
                    <div class="line-nav">
                        <a href="javascript:void(0)" v-for="status in selects.status" :key="status.id"
                           :class="{active:status.isStatus}" @click="statusHandler(status)">{{status.label}}</a>
            ...
            <el-col :span="9" :offset="6">
                <div class="searchselect">
                    ...
                    <el-select v-model="query.contractId" placeholder="请选择合同" filterable clearable
                               @change="getAdvanceList()" class="dialogselect">
                        <el-option label="全部" value=""></el-option>
                        <el-option
        ...
        <erp-table :header="header" :content="dataList" @currentPage="getCurrentPage"
                   @pageSize="getpageSize"></erp-table>
        <erp-dialog v-loading.fullscreen="dialog.loading" :title="dialog.param.id?'预付款编辑':'预付款收取'"
                    :dialog="dialog"></erp-dialog>
    </con-head>
