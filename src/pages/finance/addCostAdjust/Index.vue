...
export default {
  ...
  methods: {
    checkUserHandler(id){ //根据商户id查询 合同列表
        this.$api.rentapi.getListForPageUsingGET({merchantId:id}).then(res=>{
            this.selects.contracts = res.data.data.list;
        }).catch(res=>{
            this.$message.error(res.data.msg);
        });
    },
    checkHetongHandler(id){ //根据合同id查询 结算组别列表
        this.$api.rentapi.getIrregularCostInfoUsingGET({contractId: id, settleGroupId: -1}).then(res=>{
    ...
    checkjsHandler(id){
        this.$api.rentapi.getIrregularCostInfoUsingGET({contractId: this.query.contractId, settleGroupId: id}).then(res=>{
