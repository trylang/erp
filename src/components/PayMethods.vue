<template>
  <div class="pay">
    <header>
      <span v-if="type ==='singleSelected'">待收费：<strong>{{needBillInfo.amountReceived}}</strong></span> 
      <span v-if="type ==='mulSelected'">待收费：<strong>{{needItemInfo.owed}}</strong></span> 
    </header>
    <section>
      ...
      <div v-if="type ==='singleSelected' && variables.isActive==='prepaid'" class="pay_content">
        <span>账户余额</span>
        <strong>{{needBillInfo.bankBalance}}</strong>
      ...
      <div v-if="type ==='mulSelected'? true : variables.isActive==='cash'" class="pay_content">
        ...
        <small-collection v-if="type ==='mulSelected'" :title="variables.title">
          <ul slot="cash" class="cash_methods"> 
            <li v-for="(item, index) in payMethods.icons" :class="{active: item.isChecked}" :key="index" @click="mulSelected(item, $event)">
        ...
        <small-collection v-if="type ==='singleSelected'" :title="variables.title">
          ...
          <ul slot="cash" class="cash_methods">
            <li v-for="(item, index) in payMethods.icons" :key="index" :class="{active: item.id === variables.isSelected}"
                @click="single_cash_methods(item)">
              <i class="iconfont" :class="item.icon" :style="{color: item.color}"></i>
              <span>{{item.label}}</span>
            </li>
          </ul>
...
export default {
  name: "pay-methods",
  props: ["needBillInfo", "needItemInfo", "dialog", "type"],
  components: {
    SmallCollection,
    CashTable
  },
  ...
  methods: {
    ...
    selectPaymentWayType(item) {
      ...
      this.dialog.param.paymentWayType = item.value ? item.value : 11;
    },
    // payType
    ...
    async getBanks() {
      await queryDicsByCode("0007").then(res => {
        ...
        res.data.forEach(item => {
          ...
          item.icon = json[item.name].icon;
          item.color = json[item.name].color;
          item.isChecked = false;
        });
    ...
    async init() {
      ...
      if (this.type === "singleSelected") {
        // 一次性收款
        this.dialog.param.paymentWayType = 11; // 初始化 付款类型 为预付款
      } else {
      }
    }
  },
  mounted() {
    if (this.type === "singleSelected") {
      this.dialog.param.paymentWayType = 11;
      this.payMethods.cash.money = this.needBillInfo.amountReceived;
    } else {
      // this.payMethods.cash.money = this.needBillInfo.bankBalance;
    }
    ...
    this.$root.eventEmit.$on("EDIT_PARAM", info => {
      ...
      let pay_methods = Object.keys(info.needItemInfo.payType);      
